...
{
    // Enemy controller for the Gunner enemy type.
    ...
    private void Start()
    {
        ...
        stateMachine.ChangeState(new State_Patrol2(this));
    }
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        {
            stateMachine.ChangeState(new State_Damaged2(this));
        }
    }
    ...
    public void Fire()
    {
        // Gunner small gun attack function.
        if (Time.time > nextFire)
        {
            nextFire = Time.time + fireRate;
            Quaternion rotation = Quaternion.LookRotation(lastSeenPosition - shotTransform1.position, Vector3.up);
            Instantiate(bullet, shotTransform1.position, rotation);
        }
